...
// AdminRepository Adminユーザー情報に関するデータベース操作を担当するリポジトリ
...
// Create 新しいAdminユーザー情報をデータベースに登録する
...
// FindByID AdminユーザーID（主キー）を指定して、該当するAdminユーザー情報を1件取得する
...
// FindByEmail メールアドレスを指定して、該当するAdminユーザー情報を1件取得する（ログイン・重複チェック用）
...
// FindAll 登録されているすべてのAdminユーザー情報をID昇順で取得する（ページングなし）
...
// FindAllWithPagination 指定された件数（limit）と開始位置（offset）に応じて、Adminユーザー情報をID昇順で取得する
...
// CountAll ページングの総ページ数計算のため、条件に合致するAdminユーザー情報の総件数を取得する
...
// Update 既存のAdminユーザー情報（名前、メール、パスワード、権限など）を更新する
...
// Delete AdminユーザーIDを指定して、該当するAdminユーザー情報を物理削除する
